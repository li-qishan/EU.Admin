...
namespace EU.Web.Controllers.AP
{
    ...
    public class ApPrepaidOrderController : BaseController1<ApPrepaidOrder>
    {
        /// <summary>
        /// 采购预付款
        /// </summary>
        ...
        public ApPrepaidOrderController(DataContext _context, IBaseCRUDVM<ApPrepaidOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ApPrepaidOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("ApPrepaidOrderNo");
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("ApPrepaidOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(UserId.ToString(), "PD_ORDER_MNG", "ApPrepaidOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.ApPrepaidOrder.Where(x => x.ID == Id).SingleOrDefault();
