...
namespace EU.Web.Controllers.AP
{
    ...
    public class ApInitAccountDetailController : BaseController<ApInitAccountDetail>
    {
        ...
        public ApInitAccountDetailController(DataContext _context, IBaseCRUDVM<ApInitAccountDetail> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ApInitAccountDetail Model)
        {
            ...
            try
            {
                ...
                Model.SerialNumber = Utility.GenerateContinuousSequence("ApInitAccountDetail", "SerialNumber", "OrderId", Model.OrderId.ToString());

                #region 税额计算
                ApInitAccountOrder order = _context.ApInitAccountOrder.Where(O => O.ID == Model.OrderId).SingleOrDefault();
                if (order != null)
                {
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult BatchAdd(List<ApInitAccountDetail> data)
        {
        ...
        public override IActionResult Update(dynamic modelModify)
        {
            ...
            try
            {
                ...
                #region 税额计算
                ApInitAccountOrder order = _context.ApInitAccountOrder.Where(O => O.ID == orderId).SingleOrDefault();
                if (order != null)
                {
                    Supplier supplier = _context.BdSupplier.Where(O => O.ID == order.SupplierId).SingleOrDefault();
                    if (supplier != null)
                    {
                        ApInitAccountDetail Model = new ApInitAccountDetail();
                        //比如税率13%情况下，客户按未税价计算，含税金额=单价x数量x1.13，未税金额=单价x数量
                        //比如税率13 % 情况下，客户按含税价计算，含税金额 = 单价x数量，未税金额 = 单价x数量 / 1.13
                        //零税
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                ...
                ApInitAccountDetail Model = _context.ApInitAccountDetail.Where(x => x.ID == Id).SingleOrDefault();
                if (Model != null)
