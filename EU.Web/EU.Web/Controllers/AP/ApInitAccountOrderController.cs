...
namespace EU.Web.Controllers.AP
{
    ...
    public class ApInitAccountOrderController : BaseController<ApInitAccountOrder>
    {
        ...
        public ApInitAccountOrderController(DataContext _context, IBaseCRUDVM<ApInitAccountOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ApInitAccountOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("ApInitAccountOrderNo");
        ...
        public override async Task<IActionResult> GetById(Guid Id)
        {
            ...
            try
            {
                ...
                ApInitAccountOrder order = await _context.ApInitAccountOrder.AsNoTracking().Where(O => O.ID == Id).FirstOrDefaultAsync();
                obj.data = order;

                Supplier supplier = await _context.BdSupplier.Where(O => O.ID == order.SupplierId).FirstOrDefaultAsync();
                if (supplier != null)
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("ApInitAccountOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(UserId.ToString(), "PD_ORDER_MNG", "ApInitAccountOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.ApInitAccountOrder.Where(x => x.ID == Id).SingleOrDefault();
