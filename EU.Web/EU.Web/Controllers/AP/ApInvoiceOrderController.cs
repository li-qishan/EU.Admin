...
namespace EU.Web.Controllers.AP
{
    ...
    public class ApInvoiceOrderController : BaseController1<ApInvoiceOrder>
    {
        /// <summary>
        /// 应付发票单
        /// </summary>
        ...
        public ApInvoiceOrderController(DataContext _context, IBaseCRUDVM<ApInvoiceOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ApInvoiceOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("ApInvoiceOrderNo");
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("ApInvoiceOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(UserId.ToString(), "PD_ORDER_MNG", "ApInvoiceOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.ApInvoiceOrder.Where(x => x.ID == Id).SingleOrDefault();
