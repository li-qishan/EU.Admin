...
namespace EU.Web.Controllers.PO
{
    ...
    public class InOrderController : BaseController1<InOrder>
    {
        /// <summary>
        /// 采购入库单
        /// </summary>
        ...
        public InOrderController(DataContext _context, IBaseCRUDVM<InOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }

        #region 新增重写
        [HttpPost]
        public override IActionResult Add(InOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("PoInOrderNo");
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("PoInOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(User.Identity.Name, "PO_IN_ORDER_MNG", "PoInOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.PoInOrder.Where(x => x.ID == Id).SingleOrDefault();
        ...
        public IActionResult ConfirmIn(Guid Id)
        {
            ...
            try
            {
                ...
                sql = string.Format(sql, Id);
                List<InOrderDetail> list = DBHelper.Instance.QueryList<InOrderDetail>(sql);
                ...
                #region 添加物料库存
                foreach (InOrderDetail item in list)
                {
                    ...
                    change.QTY = item.InQTY;
                    change.AfterQTY = qty + item.InQTY;
                    change.ChangeType = IVChangeHelper.ChangeType.PurchaseIn.ToString();
                    change.OrderId = item.OrderId;
                    change.OrderDetailId = item.ID;
                    IVChangeHelper.Add(change, trans);

                    sql = @"UPDATE BdMaterialInventory
                                SET QTY = QTY + '{3}', UpdateBy = '{4}', UpdateTime = GETDATE ()
                                WHERE MaterialId = '{0}' AND StockId = '{1}' AND GoodsLocationId = '{2}'";

                    sql = string.Format(sql, item.MaterialId, item.StockId, item.GoodsLocationId, item.InQTY, UserId);
                ...
                #region 导入订单操作历史
                DBHelper.RecordOperateLog(User.Identity.Name, "PO_IN_ORDER_MNG", "PoInOrder", Id.ToString(), OperateType.Update, "CompleteIn", "修改订单状态为：CompleteIn");
                #endregion
