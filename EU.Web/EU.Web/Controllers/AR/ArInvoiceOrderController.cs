...
namespace EU.Web.Controllers.AR
{
    ...
    public class ArInvoiceOrderController : BaseController1<ArInvoiceOrder>
    {
        /// <summary>
        /// 应收开票
        /// </summary>
        ...
        public ArInvoiceOrderController(DataContext _context, IBaseCRUDVM<ArInvoiceOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ArInvoiceOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("ArInvoiceOrderNo");
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("ArInvoiceOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(UserId.ToString(), "PD_ORDER_MNG", "ArInvoiceOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.ArInvoiceOrder.Where(x => x.ID == Id).SingleOrDefault();
