...
namespace EU.Web.Controllers.AR
{
    ...
    public class ArPrepaidOrderController : BaseController<ArPrepaidOrder>
    {
        ...
        public ArPrepaidOrderController(DataContext _context, IBaseCRUDVM<ArPrepaidOrder> BaseCrud) : base(_context, BaseCrud)
        {
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public override IActionResult Add(ArPrepaidOrder Model)
        {
            ...
            try
            {
                ...
                Model.OrderNo = Utility.GenerateContinuousSequence("ArPrepaidOrderNo");
        ...
        public IActionResult AuditOrder(dynamic modelModify)
        {
            ...
            try
            {
                ...
                DbUpdate du = new DbUpdate("ArPrepaidOrder");
                du.Set("AuditStatus", auditStatus);
                du.Where("ID", "=", orderId);
                DBHelper.Instance.ExecuteScalar(du.GetSql());

                #region 导入订单操作历史
                DBHelper.RecordOperateLog(UserId.ToString(), "PD_ORDER_MNG", "ArPrepaidOrder", orderId, OperateType.Update, "Audit", "修改订单审核状态为：" + auditStatus);
                #endregion
        ...
        public override IActionResult Delete(Guid Id)
        {
            ...
            try
            {
                var Order = _context.ArPrepaidOrder.Where(x => x.ID == Id).SingleOrDefault();
