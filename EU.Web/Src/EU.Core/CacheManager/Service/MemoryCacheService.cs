...
namespace EU.Core.CacheManager
{
    public class MemoryCacheService
    {
        ...
        public void Clear()
        {
            ...
            //foreach (string key in Enum.GetNames(typeof(CacheKeys)))
            //{
            //    _cache.Remove(key);
            //}
            //string sql = "SELECT DISTINCT CACHE_KEY FROM SM_CACHE_LOG";
            //List<Sm_Cache_Log> cache = DBHelper.Instance.QueryList<Sm_Cache_Log>(sql, null);
            //cache.ForEach(item => _cache.Remove(item.Cache_Key));
            //sql = "TRUNCATE TABLE SM_CACHE_LOG";
        ...
        //private void RecordCacheLog (string key)
        //{
        //    //Task.Factory.StartNew(() => UploadFileMethod(tempStream, physicalPath, folder, fileId, fileExt, oldFileName));
        //    DbInsert di = new DbInsert("SM_CACHE_LOG");
        //    di.Values("ROW_ID", StringHelper.Id);
        //    di.Values("CACHE_KEY", key);
