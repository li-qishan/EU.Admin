...
namespace EU.TaskHelper
{
    ...
    public class SchedulerCenterServer : ISchedulerCenter
    {
        ...
        public async Task<MessageModel<string>> StartScheduleAsync()
        {
            ...
            try
            {
                ...
                if (!this._scheduler.Result.IsStarted)
                {
                    ...
                    await Console.Out.WriteLineAsync($"{DateTime.Now:HH:mm:ss} " + "任务调度开启！");
                    result.success = true;
                    result.msg = $"任务调度开启成功";
                    return result;
                }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<MessageModel<string>> InitJobAsync()
        {
            var result = new MessageModel<string>();
            try
            {
                Logger.WriteLog($"开始初始化任务！");
                var systemTaskItems = TaskHelper.GetSmQuartzJobs().ToList();
                ...
                //var jobs = new List<TasksQz>
                //{
                //    new TasksQz()
                //    {
                //        Id = Guid.Parse("c143828e-0f87-4343-b001-d2b0a1490770"),
                //        Name = "测试1",
                //        JobGroup = "1111",
                ...
                foreach (var item in jobs)
                {
                    try
                    {
                        ...
                        if (result1.success)
                        {
                            ...
                            Logger.WriteLog($"QuartzNetJob {item.Name} {m_NextTime?.ToString("yyyy-MM-dd HH:mm:ss")} 距离下次执行剩余 {m_Lefttime} s");
                        }
                        else
                    ...
                    {
                        result.msg = $"初始化计划任务失败:【{ex.Message}】";
                    }
                }
                Logger.WriteLog($"初始化任务结束！");

                result.msg = $"初始化计划任务成功！";
                //LoggerHelper.SendLog($"QuartzNetJob 结束初始化任务！");
            }
            catch (Exception ex)
            {
                result.msg = $"初始化计划任务失败:【{ex.Message}】";
