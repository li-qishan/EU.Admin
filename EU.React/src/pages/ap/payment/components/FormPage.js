...
import DetailTable from '../../paymentdetail/components/TableList';
import WriteOff from '../../paymentwriteoff/components/TableList';
...
class FormPage extends Component {
    ...
    constructor(props) {
        ...
        me.state = {
            Id: '',
            moduleCode: "AP_PAYMENT_ORDER_MNG",
            tabkey: 1,
            isLoading: true,
            detailCount: 0,
            IsView: false,
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'appayment/saveData',
            payload: values,
        }).then((result) => {
            const { appayment: { Id } } = me.props;
            if (Id)
            ...
            >
                {this.state.isLoading ? <Card>
                    ...
                    <Card>
                        ...
                        <Row gutter={24} justify={"center"}>
                            <Col span={8}>
                                <FormItem name="Amount" label="应付金额">
                                    <InputNumber placeholder="请输入" disabled={true} />
                                </FormItem>
                            </Col>
                            <Col span={8}>
                                <FormItem name="ActualAmount" label="实付金额">
                                    <InputNumber placeholder="请输入" disabled={true} />
                                </FormItem>
                            </Col>
                            <Col span={8}>
                                <FormItem name="DiffAmount" label="付款差额">
                                    <InputNumber placeholder="请输入" disabled={true} />
                                </FormItem>
                            </Col>
                        </Row>
...
export default connect(({ appayment, user }) => ({
    appayment, user
}))(FormPage);
