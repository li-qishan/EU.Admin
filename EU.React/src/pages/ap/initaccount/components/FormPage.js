...
import DetailTable from '../../initaccountdetail/components/TableList';
...
class FormPage extends Component {
    ...
    constructor(props) {
        ...
        me.state = {
            Id: '',
            moduleCode: "AP_INIT_ACCOUNT_ORDER_MNG",
            tabkey: 1,
            isLoading: true,
            detailCount: 0,
            IsView: false,
            modelName: 'apinitaccount'
        };
    }
    componentWillMount() {
        ...
        if (dispatch && Id) {
            ...
            dispatch({
                ...
                if (data && me.formRef.current) {
                    Utility.setFormFormat(data.data);
                    me.formRef.current.setFieldsValue(data.data);
                    dispatch({
                        type: 'apinitaccount/setDetail',
                        payload: data
                    });
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'apinitaccount/saveData',
            payload: values,
        }).then((result) => {
            const { apinitaccount: { Id } } = me.props;
            if (Id)
            ...
            >
                {this.state.isLoading ? <Card>
                    ...
                    <Card>
                        ...
                        <Row gutter={24} justify={"center"}>
                            <Col span={8}>
                                <FormItem name="InvoiceTitle" label="发票抬头">
...
export default connect(({ apinitaccount, user }) => ({
    apinitaccount, user
}))(FormPage);
