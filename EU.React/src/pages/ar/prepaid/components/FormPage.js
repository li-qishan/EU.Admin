...
class FormPage extends Component {
    ...
    constructor(props) {
        ...
        me.state = {
            Id: '',
            moduleCode: "AR_PREPAID_ORDER_MNG",
            tabkey: 1,
            isLoading: true,
            detailCount: 0,
            IsView: false,
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'arprepaid/saveData',
            payload: values,
        }).then((result) => {
            const { arprepaid: { Id } } = me.props;
            if (Id)
...
export default connect(({ arprepaid, user }) => ({
    arprepaid, user
}))(FormPage);
