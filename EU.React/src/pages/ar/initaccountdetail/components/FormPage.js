...
class FormPage extends Component {
    ...
    static getDerivedStateFromProps(nextProps) {
        if (me.props.modalVisible !== nextProps.modalVisible && nextProps.modalVisible) {
            ...
            const { arinitaccountdetail: { Id } } = me.props;
            if (dispatch && Id && modalVisible) {
                dispatch({
                    type: 'arinitaccountdetail/getById',
                    payload: { Id },
                }).then((result) => {
    ...
    okHandle = async (value) => {
        ...
        const { arinitaccountdetail: { Id }, arinitaccount: { detail: { extend: { TaxRate } } } } = me.props;
        if (Id)
            fieldsValue.ID = Id;
    ...
    onSubmitSuccess() {
        ...
        me.formRef.current.resetFields();
        dispatch({
            type: 'arinitaccountdetail/setId',
            payload: ''
            // }).then(() => {
    ...
    //     }
    //     dispatch({
    //         type: 'arinitaccountdetail/saveData',
    //         payload: values,
    //     }).then(() => {
    //         const { dispatch, arinitaccountdetail: { Id } } = me.props;
    //         this.setState({ Id })
    //     });arinitaccount detail:{extend:TaxRate }
    // }
    render() {
        const { modalVisible, IsView, onCancel, arinitaccountdetail: { Id }, arinitaccount: { detail: { extend: { TaxRate } } } } = this.props;
...
export default connect(({ arinitaccountdetail, arinitaccount }) => ({
    arinitaccountdetail, arinitaccount
}))(FormPage);
