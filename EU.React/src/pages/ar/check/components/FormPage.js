...
class FormPage extends Component {
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'archeck/saveData',
            payload: values,
        }).then((result) => {
            const { archeck: { Id } } = me.props;
            if (Id)
    ...
    async onGenerateDetail() {
        ...
        Modal.confirm({
            ...
            async onOk() {
                ...
                let result = await request('/api/archeckDetail/GenerateDetail/' + Id, {
                    method: 'POST'
                });
                setTimeout(filehide);
...
export default connect(({ archeck, user }) => ({
    archeck, user
}))(FormPage);
