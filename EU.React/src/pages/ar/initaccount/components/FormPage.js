...
class FormPage extends Component {
    ...
    constructor(props) {
        ...
        me.state = {
            Id: '',
            moduleCode: "AR_INIT_ACCOUNT_ORDER_MNG",
            tabkey: 1,
            isLoading: true,
            detailCount: 0,
            IsView: false,
            modelName: 'arinitaccount'
        };
    }
    componentWillMount() {
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'arinitaccount/saveData',
            payload: values,
        }).then((result) => {
            const { arinitaccount: { Id } } = me.props;
            if (Id)
...
export default connect(({ arinitaccount, user }) => ({
    arinitaccount, user
}))(FormPage);
