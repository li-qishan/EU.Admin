...
import DetailTable from '../../actualcheckdetail/components/TableList';
...
class FormPage extends Component {
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'ivactualcheck/saveData',
            payload: values,
        }).then(() => {
            const { ivactualcheck: { Id } } = me.props;
            if (Id)
    ...
    async onConfirmCheck() {
        ...
        Modal.confirm({
            ...
            async onOk() {
                ...
                let result = await request('/api/ivactualcheck/ConfirmCheck', {
                    method: 'POST',
                    params: {
                        Id
                    }
    ...
    async generateCheckDetail() {
        ...
        Modal.confirm({
            ...
            async onOk() {
                const filehide = message.loading('数据提交中..', 0)
                let result = await request('/api/ivactualcheck/GenerateCheckDetail', {
                    method: 'POST',
                    params: {
                        Id
                    }
...
export default connect(({ ivactualcheck, user }) => ({
    ivactualcheck, user
}))(FormPage);
