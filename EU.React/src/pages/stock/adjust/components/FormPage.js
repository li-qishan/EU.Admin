...
class FormPage extends Component {
    ...
    onFinish(values) {
        ...
        dispatch({
            type: 'ivadjust/saveData',
            payload: values,
        }).then(() => {
            const { ivadjust: { Id } } = me.props;
            if (Id)
    ...
    async onAdjust() {
        ...
        Modal.confirm({
            ...
            async onOk() {
                ...
                let result = await request('/api/IvAdjust/ConfirmAdjust', {
                    method: 'POST',
                    params: {
                        Id
                    }
                ...
                if (result && result.status == "ok") {
                    me.setState({ AuditStatus: 'CompleteAdjust' });
                    Modal.success({
...
export default connect(({ ivadjust, user }) => ({
    ivadjust, user
