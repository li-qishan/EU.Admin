...
class FormPage extends Component {
    ...
    static getDerivedStateFromProps(nextProps) {
        if (me.props.modalVisible !== nextProps.modalVisible) {
            const { ivadjustdetail: { Id } } = me.props;
    ...
    okHandle = async (value) => {
        ...
        const { ivadjustdetail: { Id } } = this.props;
        if (Id)
            fieldsValue.ID = Id;
        if (value == "newSave")
    ...
    //         payload: values,
    //     }).then(() => {
    //         const { dispatch, ivadjustdetail: { Id } } = me.props;
    //         this.setState({ Id })
    //     });
    // }
    render() {
        const { modalVisible, IsView, onCancel, ivadjustdetail: { Id } } = this.props;
                ...
                >
                    {isLoading ? <Card>
                        ...
                        <Row gutter={24} justify={"center"}>
                            <Col span={8}>
                                <FormItem name="AdjustType" label="调整类型">
