...
class FormPage extends Component {
    ...
    okHandle = async () => {
        ...
        selectList.forEach(record => {
            ...
            if (!record.OutStockId || !record.OutGoodsLocationId || !record.InStockId || !record.InGoodsLocationId)
                flag = 1;
            if (record.OutGoodsLocationId && record.InGoodsLocationId && record.OutGoodsLocationId == record.InGoodsLocationId)
                flag1 = 1;
            if (record.QTY <= 0)
                flag2 = 1;
        });
    ...
    onMatchAll = async () => {
        ...
        result.data.forEach((record) => {
            ...
            record.OutStockId = "";
            record.OutStockList = StockList;
            record.OutGoodsLocationId = "";
            record.OutGoodsLocationList = [];
            record.InStockId = "";
            record.InStockList = StockList;
            record.InGoodsLocationId = "";
            record.InGoodsLocationList = [];
            record.QTY = 1;
            record.Price = 1;
    ...
    async onChangeStock(values, record) {
        ...
        if (flag > -1) {
            ...
            tempList[flag].OutGoodsLocationId = null;
            tempList[flag].OutGoodsLocationList = [];

            if (result && result.status == "ok" && result.data && result.data.length > 0) {
                tempList[flag].OutGoodsLocationId = result.data[0].ID;
                tempList[flag].OutGoodsLocationList = result.data;
            }
            me.setState({ selectList: tempList }, () => {
            })
        }
    }
    async onChangeInStock(values, record) {
        let { selectList } = me.state;
        let tempList = [...selectList];
        let flag = -1;
        for (let i = 0; i < tempList.length; i++)
        ...
        if (flag > -1) {
            ...
            tempList[flag].InGoodsLocationId = null;
            tempList[flag].InGoodsLocationList = [];
            if (result && result.status == "ok" && result.data && result.data.length > 0) {
                tempList[flag].InGoodsLocationId = result.data[0].ID;
                tempList[flag].InGoodsLocationList = result.data;
            }
            me.setState({ selectList: tempList }, () => {
            })
        }
    ...
    async onChangeLoction(values, record) {
        ...
        for (let i = 0; i < tempList.length; i++)
            if (tempList[i].ID == record.ID) {
                flag = i;
                tempList[i].OutGoodsLocationId = values;
            }
        me.setState({ selectList: tempList });
    }
    async onChangeInLoction(values, record) {
        let { selectList } = this.state;
        let tempList = [...selectList];
        let flag = -1;
        for (let i = 0; i < tempList.length; i++)
            if (tempList[i].ID == record.ID) {
                flag = i;
                tempList[i].InGoodsLocationId = values;
            }
        me.setState({ selectList: tempList });
    }
    render() {
        ...
        const hasColumns = [{
            ...
            render: (text, record) => (
                ...
                >
                    {record && record.OutStockList.length > 0 && record.OutStockList.map(item => {
                        return (
            ...
            render: (text, record) => (
                ...
                >
                    {record && record.OutGoodsLocationList.length > 0 && record.OutGoodsLocationList.map(item => {
                        return (
            ...
            render: (text, record) => (
                <Select
                    ...
                    filterOption={false}
                    value={text}
                    onChange={(value, Option) => me.onChangeInStock(value, record)}
                >
                    {record && record.InStockList.length > 0 && record.InStockList.map(item => {
                        return (
                            <Option key={item['ID']}>{item['StockNames']}</Option>
                        )
                    })}
            ...
            render: (text, record) => (
                <Select
                    ...
                    filterOption={false}
                    value={text}
                    onChange={(value, Option) => me.onChangeInLoction(value, record)}
                >
                    {record && record.InGoodsLocationList.length > 0 && record.InGoodsLocationList.map(item => {
                        return (
                            <Option key={item['ID']}>{item['LocationNames']}</Option>
                        )
                    })}
